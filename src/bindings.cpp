...
Supports LM and L-BFGS optimizers with analytic Jacobians.
    ...
    // ---- ClothVBDSolver (VBD for triangle cloth) ----------------------------

    py::class_<chysx::solver::VBDSolver>(m, "ClothVBDSolver", R"pbdoc(
Vertex Block Descent solver for cloth (triangle membrane + dihedral bending).
)pbdoc")
        .def(py::init<>())

        .def("build_cloth_coloring",
            [](chysx::solver::VBDSolver& self,
               py::array_t<int> tri_indices,
               py::array_t<int> edge_indices,
               int n_particles) {
                auto tri_buf = tri_indices.request();
                auto edge_buf = edge_indices.request();
                int n_tris = static_cast<int>(tri_buf.shape[0]);
                int n_edges = static_cast<int>(edge_buf.shape[0]);
                self.build_cloth_coloring(
                    reinterpret_cast<const chysx::math::Vec3i*>(tri_buf.ptr), n_tris,
                    reinterpret_cast<const chysx::math::Vec4i*>(edge_buf.ptr), n_edges,
                    n_particles);
            },
            py::arg("tri_indices"), py::arg("edge_indices"), py::arg("n_particles"),
            "Build graph coloring from triangle + bending edge topology.")

        .def("build_tri_adjacency",
            [](chysx::solver::VBDSolver& self,
               py::array_t<int> tri_indices, int n_particles) {
                auto buf = tri_indices.request();
                int n_tris = static_cast<int>(buf.shape[0]);
                self.build_tri_adjacency(
                    reinterpret_cast<const chysx::math::Vec3i*>(buf.ptr), n_tris,
                    n_particles);
            },
            py::arg("tri_indices"), py::arg("n_particles"),
            "Build per-vertex triangle adjacency (CSR).")

        .def("build_edge_adjacency",
            [](chysx::solver::VBDSolver& self,
               py::array_t<int> edge_indices, int n_particles) {
                auto buf = edge_indices.request();
                int n_edges = static_cast<int>(buf.shape[0]);
                self.build_edge_adjacency(
                    reinterpret_cast<const chysx::math::Vec4i*>(buf.ptr), n_edges,
                    n_particles);
            },
            py::arg("edge_indices"), py::arg("n_particles"),
            "Build per-vertex bending-edge adjacency (CSR).")

        .def("step_cloth",
            [](chysx::solver::VBDSolver& self,
               std::uintptr_t pos_ptr, std::uintptr_t vel_ptr,
               std::uintptr_t inv_mass_ptr, int n_particles,
               std::uintptr_t tri_idx_ptr, std::uintptr_t tri_poses_ptr,
               std::uintptr_t tri_areas_ptr, std::uintptr_t tri_mats_ptr,
               int n_tris,
               std::uintptr_t edge_idx_ptr, std::uintptr_t edge_rest_angle_ptr,
               std::uintptr_t edge_rest_length_ptr, std::uintptr_t edge_props_ptr,
               int n_edges,
               float gx, float gy, float gz,
               float dt, int iterations, std::uintptr_t stream) {
                using namespace chysx::math;
                chysx::DeviceSpan<Vec3f> pos(reinterpret_cast<Vec3f*>(pos_ptr), n_particles);
                chysx::DeviceSpan<Vec3f> vel(reinterpret_cast<Vec3f*>(vel_ptr), n_particles);
                chysx::DeviceSpan<float> inv_mass(reinterpret_cast<float*>(inv_mass_ptr), n_particles);

                self.step_cloth(
                    pos, vel, inv_mass,
                    reinterpret_cast<const Vec3i*>(tri_idx_ptr), n_tris,
                    reinterpret_cast<const Mat2f*>(tri_poses_ptr),
                    reinterpret_cast<const float*>(tri_areas_ptr),
                    reinterpret_cast<const float*>(tri_mats_ptr),
                    reinterpret_cast<const Vec4i*>(edge_idx_ptr), n_edges,
                    reinterpret_cast<const float*>(edge_rest_angle_ptr),
                    reinterpret_cast<const float*>(edge_rest_length_ptr),
                    reinterpret_cast<const float*>(edge_props_ptr),
                    Vec3f(gx, gy, gz), dt, iterations, stream);
            },
            py::arg("pos_ptr"), py::arg("vel_ptr"),
            py::arg("inv_mass_ptr"), py::arg("n_particles"),
            py::arg("tri_indices_ptr"), py::arg("tri_poses_ptr"),
            py::arg("tri_areas_ptr"), py::arg("tri_materials_ptr"),
            py::arg("n_tris"),
            py::arg("edge_indices_ptr"), py::arg("edge_rest_angle_ptr"),
            py::arg("edge_rest_length_ptr"), py::arg("edge_bending_props_ptr"),
            py::arg("n_edges"),
            py::arg("gx"), py::arg("gy"), py::arg("gz"),
            py::arg("dt"), py::arg("iterations"),
            py::arg("cuda_stream") = 0,
            "Run one cloth VBD substep.")

        .def("num_colors", &chysx::solver::VBDSolver::num_colors);
}
