...
// -----------------------------------------------------------------------
// Cloth VBD solve kernel — triangle membrane + dihedral bending
// -----------------------------------------------------------------------
}  // anonymous namespace

#include "cloth_vbd_kernels.cuh"

namespace {

__global__ void vbd_cloth_solve_kernel(
    const int* __restrict__ color_ids,
    int n_color,
    float dt,
    math::Vec3f* __restrict__ pos,
    ...
    const math::Vec3f* __restrict__ inertia,
    // triangle data
    const math::Vec3i* __restrict__ tri_indices,
    const math::Mat2f* __restrict__ tri_poses,
    const float* __restrict__ tri_areas,
    const float* __restrict__ tri_materials,    // [n_tris*3]: mu, lmbd, damp
    const int* __restrict__ tri_adj_offsets,
    const int* __restrict__ tri_adj_data,
    // bending edge data
    const math::Vec4i* __restrict__ edge_indices,
    const float* __restrict__ edge_rest_angle,
    const float* __restrict__ edge_rest_length,
    const float* __restrict__ edge_bending_props,  // [n_edges*2]: stiffness, damp
    const int* __restrict__ edge_adj_offsets,
    const int* __restrict__ edge_adj_data,
    // output
    math::Vec3f* __restrict__ displacements)
{
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // Triangle membrane (Stable Neo-Hookean)
    if (tri_indices != nullptr) {
        const int tri_begin = tri_adj_offsets[pi];
        const int tri_end = tri_adj_offsets[pi + 1];
        for (int a = tri_begin; a < tri_end; a += 2) {
            const int tri_id = tri_adj_data[a];
            const int v_order = tri_adj_data[a + 1];

            const float mu    = tri_materials[tri_id * 3 + 0];
            const float lmbd  = tri_materials[tri_id * 3 + 1];
            const float damp  = tri_materials[tri_id * 3 + 2];

            if (mu <= 0.0f && lmbd <= 0.0f) continue;

            const math::Vec3i& ti = tri_indices[tri_id];
            const math::Vec3f x0 = pos[ti.x], x1 = pos[ti.y], x2 = pos[ti.z];
            const math::Vec3f x0p = q_prev[ti.x], x1p = q_prev[ti.y], x2p = q_prev[ti.z];

            cloth_vbd_membrane_force_hessian(
                v_order, x0, x1, x2, x0p, x1p, x2p,
                tri_poses[tri_id], tri_areas[tri_id],
                mu, lmbd, damp, dt, f, h);
        }
    }
    ...
    if (edge_indices != nullptr) {
        const int edge_begin = edge_adj_offsets[pi];
        const int edge_end = edge_adj_offsets[pi + 1];
        for (int a = edge_begin; a < edge_end; a += 2) {
            const int edge_id = edge_adj_data[a];
            const int v_order = edge_adj_data[a + 1];

            const float stiffness = edge_bending_props[edge_id * 2 + 0];
            const float damp      = edge_bending_props[edge_id * 2 + 1];

            if (stiffness <= 0.0f) continue;

            const math::Vec4i& ei = edge_indices[edge_id];
            if (ei.x < 0 || ei.y < 0) continue;  // boundary edge

            const math::Vec3f x0 = pos[ei.x], x1 = pos[ei.y];
            const math::Vec3f x2 = pos[ei.z], x3 = pos[ei.w];
            const math::Vec3f x0p = q_prev[ei.x], x1p = q_prev[ei.y];
            const math::Vec3f x2p = q_prev[ei.z], x3p = q_prev[ei.w];

            cloth_vbd_bending_force_hessian(
                v_order, x0, x1, x2, x3, x0p, x1p, x2p, x3p,
                edge_rest_angle[edge_id], edge_rest_length[edge_id],
                stiffness, damp, dt, f, h);
        }
    }

    // Local 3×3 solve: displacement += H^{-1} * f
    const float det_h = determinant(h);
    if (fabsf(det_h) > 1.0e-8f) {
        const math::Mat3f h_inv = inverse(h);
        displacements[pi] = displacements[pi] + h_inv * f;
...
void VBDSolver::build_cloth_coloring(
    const math::Vec3i* host_tris, int n_tris,
    const math::Vec4i* host_edges, int n_edges,
    int n_particles) {

    n_particles_ = n_particles;

    // Build conflict graph: triangle edges + bending C(4,2) pairs
    std::set<std::pair<int,int>> edge_set;

    // Triangle edges
    for (int t = 0; t < n_tris; ++t) {
        const int v[3] = { host_tris[t].x, host_tris[t].y, host_tris[t].z };
        for (int a = 0; a < 3; ++a) {
            for (int b = a + 1; b < 3; ++b) {
                int u = v[a], w = v[b];
                if (u > w) std::swap(u, w);
    ...
    // Bending edge pairs (all C(4,2) from [o0, o1, v2, v3])
    for (int e = 0; e < n_edges; ++e) {
        const int v[4] = { host_edges[e].x, host_edges[e].y,
                           host_edges[e].z, host_edges[e].w };
        for (int a = 0; a < 4; ++a) {
            if (v[a] < 0) continue;
            for (int b = a + 1; b < 4; ++b) {
                if (v[b] < 0) continue;
...
void VBDSolver::build_tri_adjacency(
    const math::Vec3i* host_tris, int n_tris, int n_particles) {

    std::vector<int> counts(n_particles, 0);
    for (int t = 0; t < n_tris; ++t) {
        ++counts[host_tris[t].x];
        ++counts[host_tris[t].y];
        ++counts[host_tris[t].z];
    }
    ...
    tri_adj_.offsets.resize(n_particles + 1);
    std::memcpy(tri_adj_.offsets.cpu_data(), offsets.data(),
                (n_particles + 1) * sizeof(int));
    tri_adj_.offsets.copy_to_device();

    tri_adj_.data.resize(total);
    if (total > 0) {
        std::memcpy(tri_adj_.data.cpu_data(), data.data(), total * sizeof(int));
        tri_adj_.data.copy_to_device();
    }
}

void VBDSolver::build_edge_adjacency(
    const math::Vec4i* host_edges, int n_edges, int n_particles) {

    // Count adjacencies: each edge has 4 vertices (o0, o1, v2, v3)
    std::vector<int> counts(n_particles, 0);
    for (int e = 0; e < n_edges; ++e) {
        const int v[4] = { host_edges[e].x, host_edges[e].y,
                           host_edges[e].z, host_edges[e].w };
    ...
    edge_adj_.offsets.resize(n_particles + 1);
    std::memcpy(edge_adj_.offsets.cpu_data(), offsets.data(),
                (n_particles + 1) * sizeof(int));
    edge_adj_.offsets.copy_to_device();

    edge_adj_.data.resize(total);
    if (total > 0) {
        std::memcpy(edge_adj_.data.cpu_data(), data.data(), total * sizeof(int));
        edge_adj_.data.copy_to_device();
    }
}

void VBDSolver::step_cloth(
    DeviceSpan<math::Vec3f> pos,
    DeviceSpan<math::Vec3f> vel,
    DeviceSpan<float> inv_mass,
    const math::Vec3i* tri_indices, int n_tris,
    const math::Mat2f* tri_poses,
    const float* tri_areas,
    const float* tri_materials,
    const math::Vec4i* edge_indices, int n_edges,
    const float* edge_rest_angle,
    const float* edge_rest_length,
    const float* edge_bending_props,
    math::Vec3f gravity,
    float dt,
    int iterations,
    std::uintptr_t cuda_stream) {
    ...
    // Phase 1: forward_step (reuse the tet VBD kernel)
    vbd_forward_step_kernel<<<grid, block, 0, stream>>>(
        pos.data(), vel.data(), inv_mass.data(),
        q_prev_.gpu_data(), inertia_.gpu_data(),
        displacements_.gpu_data(), mass_.gpu_data(),
        gravity, dt, n);
    check_cuda(cudaGetLastError(), "vbd_forward_step_kernel (cloth)");
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        for (int c = 0; c < static_cast<int>(color_groups_.size()); ++c) {
            ...
            const int grid_c = (n_color + block - 1) / block;
            vbd_cloth_solve_kernel<<<grid_c, block, 0, stream>>>(
                color_groups_[c].indices.gpu_data(),
                n_color,
                dt,
                pos.data(),
                ...
                inertia_.gpu_data(),
                // triangle
                (n_tris > 0) ? tri_indices : nullptr,
                (n_tris > 0) ? tri_poses : nullptr,
                (n_tris > 0) ? tri_areas : nullptr,
                (n_tris > 0) ? tri_materials : nullptr,
                tri_adj_.offsets.gpu_data(),
                tri_adj_.data.gpu_data(),
                // bending
                (n_edges > 0) ? edge_indices : nullptr,
                (n_edges > 0) ? edge_rest_angle : nullptr,
                (n_edges > 0) ? edge_rest_length : nullptr,
                (n_edges > 0) ? edge_bending_props : nullptr,
                edge_adj_.offsets.gpu_data(),
                edge_adj_.data.gpu_data(),
                // output
                displacements_.gpu_data());
            check_cuda(cudaGetLastError(), "vbd_cloth_solve_kernel");
        }
    }
    ...
    vbd_update_velocity_kernel<<<grid, block, 0, stream>>>(
        pos.data(), q_prev_.gpu_data(), vel.data(), inv_dt, n);
    check_cuda(cudaGetLastError(), "vbd_update_velocity_kernel (cloth)");
}
