...
// CSR-style per-vertex adjacency to elements.
// For vertex i, adjacent elements are stored at indices
// [offsets[i], offsets[i+1]) in the flat `data` array.
// Each entry is a pair: (element_id, vertex_order_in_element).
...
public:
    ...
    // ---- cloth-specific setup ------------------------------------------

    // Build graph coloring from triangle + bending edge topology.
    // `host_tris` is (T, 3) int32, `host_edges` is (E, 4) int32 [o0,o1,v2,v3].
    void build_cloth_coloring(
    ...
    // Run one complete cloth VBD substep (triangle membrane + dihedral bending).
    void step_cloth(
        DeviceSpan<math::Vec3f> pos,
        DeviceSpan<math::Vec3f> vel,
        DeviceSpan<float> inv_mass,
        ...
        const math::Mat2f* tri_poses,     // Dm^-1 per triangle
        const float* tri_areas,           // rest area per triangle
        const float* tri_materials,       // [n_tris * 3]: mu, lambda, damping
        // bending edge data
        const math::Vec4i* edge_indices,  int n_edges,
        const float* edge_rest_angle,
        const float* edge_rest_length,
        const float* edge_bending_props,  // [n_edges * 2]: stiffness, damping
        // simulation params
        math::Vec3f gravity,
        float dt,
        int iterations,
...
private:
    ...
    // Adjacency (tet, triangle, bending edge)
    ElementAdjacency tet_adj_;
    ElementAdjacency tri_adj_;
    ElementAdjacency edge_adj_;
