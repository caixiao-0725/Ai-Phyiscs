...
// ============================================================================
// Chain scene — gear chain driven by gravity on a rail
// Matches rigid-ipc fixtures/3D/mechanisms/gears/chain.json
// ============================================================================

class ChainScene : public Scene {
public:
    const char* name() const override { return "Rigid-IPC: Chain"; }
    ...
    void setup() override {
        ...
        solver_.config.dt = 0.01f;
        solver_.config.gravity = Vec3f(0.0f, -9.8f, 0.0f);
        solver_.config.d_hat = 0.001f;
        solver_.config.contact_kappa = 0.0f;
        solver_.config.default_friction = 0.0f;
        solver_.config.newton_max_iter = 1000;
        solver_.config.total_frames = 500;
        solver_.config.export_obj = true;
        solver_.config.output_dir = "output/rigid_ipc_chain/";

        auto p = [](const char* rel) { return chysx::render::get_data_path(rel); };

        // Body 0: rail — density=100, only Y translation free
        {
            bool dof[6] = { true, false, true, true, true, true };
            solver_.add_body_from_obj(p("meshes/gears/rail.obj"), 100.0f,
                Vec3f(-1.25f, 2.25f, 0.0f), Vec3f(90, 0, -90),
                1.0f, false, dof);
        }

        // Bodies 1..5: five gears — only rz free
        struct GearDef { Vec3f pos; Vec3f rot; };
        GearDef gears[] = {
            { Vec3f(0.0f, 0.0f, 0.0f),   Vec3f(90, 0, 0) },
            { Vec3f(2.05f, 0.0f, 0.0f),  Vec3f(90, 0, 15) },
            { Vec3f(4.1f, 0.0f, 0.0f),   Vec3f(90, 0, 0) },
            { Vec3f(6.15f, 0.0f, 0.0f),  Vec3f(90, 0, 15) },
            { Vec3f(8.2f, 0.0f, 0.0f),   Vec3f(90, 0, 0) },
        };
        bool gear_dof[6] = { true, true, true, true, true, false };
        for (auto& g : gears) {
            solver_.add_body_from_obj(p("meshes/gears/gear.obj"), 1.0f,
                g.pos, g.rot, 1.0f, false, gear_dof);
        }

        rebuild_draw_buffers();
        mkdir_recursive("output/rigid_ipc_chain");

        printf("=== Rigid-IPC: Chain Scene ===\n");
        printf("Bodies: %d (0=rail, 1-5=gears)\n", solver_.num_bodies());
        for (int b = 0; b < solver_.num_bodies(); ++b) {
            auto& body = solver_.body(b);
            Vec3f pos = pose_position(body.q);
            printf("Body %d: mass=%.4f, I=(%.4e, %.4e, %.4e), "
                   "pos=(%.4f, %.4f, %.4f), fixed=%d, dof=[%d%d%d%d%d%d]\n",
                   b, body.mass,
                   body.moment_of_inertia.x, body.moment_of_inertia.y,
                   body.moment_of_inertia.z,
                   pos.x, pos.y, pos.z, body.is_fixed,
                   !body.dof_fixed[0], !body.dof_fixed[1], !body.dof_fixed[2],
                   !body.dof_fixed[3], !body.dof_fixed[4], !body.dof_fixed[5]);
        }
        printf("==============================\n");
    }
    ...
    void step(float /*dt*/) override {
        ...
        auto info = solver_.last_step_info();
        printf("[Chain] frame %d: newton=%d contacts=%d res=%.4e min_d=%.6e\n",
               frame_, info.newton_iters, info.contact_count,
               info.residual, info.min_distance);
    ...
    void draw_meshes(std::vector<DrawMesh>& out) override {
        float colors[][3] = {
            {0.6f, 0.6f, 0.6f},  // rail
            {0.8f, 0.3f, 0.1f},  // gear 1
            {0.1f, 0.5f, 0.8f},  // gear 2
            {0.2f, 0.8f, 0.3f},  // gear 3
            {0.9f, 0.8f, 0.1f},  // gear 4
            {0.6f, 0.2f, 0.7f},  // gear 5
        };
        for (int b = 0; b < solver_.num_bodies(); ++b) {
            DrawMesh dm;
            dm.positions = draw_pos_[b].data();
            ...
            dm.n_tris = static_cast<int>(draw_tri_[b].size()) / 3;
            int ci = b < 6 ? b : 0;
            dm.color_r = colors[ci][0];
            dm.color_g = colors[ci][1];
            dm.color_b = colors[ci][2];
            dm.wireframe = false;
            out.push_back(dm);
        }
    }
...
private:
    ...
    void export_frame() {
        for (int b = 0; b < solver_.num_bodies(); ++b) {
            ...
            char path[256];
            snprintf(path, sizeof(path), "output/rigid_ipc_chain/body%d_frame%04d.obj",
                     b, frame_);
            chysx::io::save_obj(path, pos.data(),
                                static_cast<int>(verts.size()),
                                tri.data(),
...
extern "C" void chysx_register_rigid_ipc_scenes() {
    ...
    register_scene("Rigid-IPC: Chain",
                   []() -> Scene* { return new ChainScene(); });
}
